# Qwen 3.7 接入轻量工具调用链路 Demo

这个 demo 面向小龙虾（OpenClaw）、Hermes 等兼容客户端用户，用来证明 WebLLM Gateway 不是普通反向代理，而是把网页登录模型接进标准工具调用协议的 Gateway。

## Demo 目标

用 Qwen Web 登录账号作为上游模型，通过 WebLLM Gateway 暴露 OpenAI / Anthropic 兼容 API，让下游客户端完成一次真实工具调用任务。这个 demo 重点验证轻中度任务，不建议用来宣传复杂编程长链路。
...
在小龙虾、Hermes 或其它兼容客户端里执行一个需要真实工具调用的轻任务，例如：

```text
请读取这份项目介绍，整理出 3 条适合发朋友圈或社群的卖点。需要先查看内容再回答。
```

理想执行过程：

1. 客户端把可用工具 schema 通过 OpenAI / Anthropic 协议发给 Gateway。
...
WebLLM Gateway 的边界是协议适配：它把网页登录模型整理成兼容客户端能理解的 OpenAI / Anthropic API，但不会替客户端执行本地工具，也不会绕过客户端权限系统。轻中度任务和低成本试错是当前更稳妥的使用方式。
